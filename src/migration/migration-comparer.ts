...
export class MigrationComparer {
	...
	async getAll() {
		...
		const migrations = migrationsRaw.reduce((acc, migration) => {
			return acc.set(migration.id, migration);
		}, new Map());

		const remoteMigrationsRaw = await this.migrationRemote.get();
		const remoteMigrations = remoteMigrationsRaw.reduce((acc, migration) => {
			return acc.set(migration.id, migration);
		}, new Map());
